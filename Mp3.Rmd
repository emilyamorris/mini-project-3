...
#Sap Season Trends
Pictured below is the total number of days on which sap could be collected plotted against typical sugaring season months. It can be seen that the number of possible collection days increases in more recent years in months that previously would have been too cold to tap in, primarily January and February. Since tapping requires temperatures above freezing during the day, this graph--along with other factors--is indicative of a warming climate. This graph represents only when it was possible to collect sap, not actual sap collection at MacLeish, which we took into account later in our analysis.
...
The analysis we did on the MacLeish data is compatible with these climate change graphics, which show that warmer temperatures are leading to an earlier and shorter sap season. 
...
These calendars show days from January to April when sap could possibly be collected, and when it was actually collected, from 2012-2017. It should be noted that there was no sap collected in 2016. The patterns of when sap was actually collected is compatible with the overall national trends of sap production. From the calendars, you can see that Smith's maple syrup ventures are actually pretty limited, and that across the years, despite the number of possible days, sap is only really collected in limited amounts. 
...
We would also recommend that data science students collaborate with MacLeish in order to improve record keeping practices, so that we have a better idea with what is going on with maple trees in the area. 
