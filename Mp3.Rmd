...
possible_and_actual_sap_days <- weather_and_all_possible_days %>%
  left_join(sap, by= "date")

```

```{r warning=FALSE, message= FALSE}
Plot_months <- function(name_arg){
  calendarPlot(possible_and_actual_sap_days, pollutant= "temp_total", year= name_arg, breaks= c(0, 1, 2), labels= c("Incorrect Temperature Ranges", "Correct Temperature Ranges"), cols= c("#d5f2f0", "#45e05f"))
}
