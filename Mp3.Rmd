...
possible_and_actual_sap_days <- weather_and_all_possible_days %>%
  ...
  select(Year:sap_gals)%>%
  replace_na(list(sap_gals= 0))

```

```{r warning=FALSE, message= FALSE}
#Function to plot the days in the right temperature range and the amount of sap actually collected
Plot_months<- function(name_arg){
  calendarPlot(possible_and_actual_sap_days, pollutant= "temp_total", year= name_arg, breaks= c(0, 1, 2), labels= c("Incorrect Temperature Ranges", "Correct Temperature Ranges"), cols= c("#d5f2f0", "#45e05f"), main= "All Possible Days")
}

Plot_actual_sap <- function(name_arg){
  calendarPlot(possible_and_actual_sap_days, pollutant= "sap_gals", breaks= c(0, 10, 20, 30, 40, 50, 60, 80, 100), year= name_arg, cols= "Blues", main= "Actual Days Collected", limits= c(0, 100))
}

plot_calendars <- function(name_arg){
  par(mfrow=c(1, 2))
  Plot_months(name_arg)
  Plot_actual_sap(name_arg)
}
```

```{r warning= FALSE, message= FALSE}
list_of_years <- list(2012, 2013, 2014, 2015, 2016, 2017)

lapply(list_of_years, plot_calendars)
```
